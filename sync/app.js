...
const areas=[{title:'היומן שלי',icon:'calendar',text:'כאן תוכלו לראות את מועדי המפגשים, לפתוח את פרטי האירוע ולהיכנס לקישור ה־<bdi>Zoom</bdi>.',shot:'היומן שלי',image:'https://tanyat-cmd.github.io/elevationhub/calendar.jpg'}, {title:'חומרי הקורס',icon:'book',text:'כאן תמצאו את חומרי הלמידה, המשימות, התרגולים, הסרטונים והמבחנים.',shot:'אזור התכנים בקורס',image:'https://tanyat-cmd.github.io/elevationhub/zfiya.jpg'}, {title:'הקלטות',icon:'play',text:'כאן תוכלו לחזור להקלטות ולחומרים ממפגשים קודמים, ככל שיהיו זמינים בקורס.',shot:'הקלטות',image:'https://tanyat-cmd.github.io/elevationhub/haklata.jpg'}, {title:'ההתקדמות שלי',icon:'progress',text:'תוכלו לראות אילו שיעורים השלמתם ובדרך כלל להמשיך מהנקודה שבה עצרתם.',shot:'ההתקדמות שלי',image:'https://tanyat-cmd.github.io/elevationhub/prog.jpg'}, {title:'סוכן ה־AI',icon:'chat',text:'סוכן הקורס יכול לענות על שאלות על בסיס חומרי הקורס ולעזור לכם למצוא את המקור הרלוונטי.',shot:'כפתור סוכן ה־AI',image:'https://tanyat-cmd.github.io/elevationhub/ai.jpg'}];
const stages=[{title:'לפני המפגש',text:'היכנסו לקורס ובדקו אם פורסמו חומרי הכנה, משימות או עדכונים לקראת המפגש.',icon:'book',shot:'חומרי ההכנה לקורס',image:'https://tanyat-cmd.github.io/elevationhub/lifnei.jpg'},{title:'בזמן המפגש',text:'פתחו את האירוע שמופיע ב״יומן שלי״ והיכנסו למפגש דרך קישור ה־<bdi>Zoom</bdi>.',icon:'calendar',shot:'היומן ופרטי האירוע',image:'https://tanyat-cmd.github.io/elevationhub/zoom.jpg'},{title:'אחרי המפגש',text:'חזרו לחומרי השיעור, השלימו את המשימות והתרגולים וצפו בהקלטה אם היא זמינה.',icon:'play',shot:'אזור התכנים בקורס',image:'https://tanyat-cmd.github.io/elevationhub/after.jpg'}];
...
function renderStage(){const s=stages[step];$('steps').innerHTML=stages.map((x,i)=>`<button class="step ${i===step?'active':i<step?'done':''}" data-step="${i}" aria-label="${x.title}${i<step?' — הושלם':''}" ${i===step?'aria-current="step"':''}><span class="num" aria-hidden="true">${i<step?icon('check'):i+1}</span><span class="step-label">${['לפני','בזמן','אחרי'][i]}</span></button>`).join('');$('stage-panel').innerHTML=`<div><span class="eyebrow">שלב ${step+1} מתוך 3</span><h2>${s.title}</h2><p>${s.text}</p></div>${visual(s)}`;$('next').innerHTML=(step===2?'לסיכום':'הבא')+' <span aria-hidden="true">←</span>'}
...
$('next').onclick=()=>{if(mode==='home'){step=0;show('flow')}else if(mode==='flow'){if(step<2){step++;renderStage()}else show('finish')}else if(mode==='finish')show('ready')};$('back').onclick=()=>{if(mode==='flow'&&step>0){step--;renderStage()}else if(mode==='flow')show('home');else if(mode==='ready')show('welcome');else if(mode==='finish'){step=2;show('flow')}};
...
cardsStyle.href = 'cards.css?v=16';
